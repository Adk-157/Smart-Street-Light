...
Web Dashboard → Database → Raspberry Pi Backend (Python) → LoRa Transmitter (via Serial) → LoRa Receiver (via RF) → I²C Slave Arduino (via I²C) → Hyovis AC Dimmer Module → Actual AC Street Light
...
| lora_transmitter.ino | Sends (status, brightness) over LoRa RF |
| lora_reciver.ino | Receives packet → forwards to I²C slave |
| i2cslave.ino | Drives AC dimmer via PWM/phase-cut control |
| Hyovis AC Dimmer Module | Handles actual mains-controlled light dimming (100–230V AC) |
...
1. **Web App → Database**: User interacts with the web interface to set the desired brightness and state for a specific street light node, which is stored in a database (e.g., Firebase, PostgreSQL).

2. **Raspberry Pi Backend**: `python_backend.py` continuously polls the database for changes and formats the control values (status, brightness) before sending them via UART serial to the LoRa transmitter Arduino.

3. **LoRa Transmitter → LoRa Receiver**: The transmitter sends the formatted data packet over the long-range RF link.

4. **LoRa RX → I²C Slave Arduino**: The receiver parses the packet and, only if the command is different from the previous state, forwards the new status and brightness values to the I²C Slave Arduino.
...
├── lora_reciver.ino         # LoRa RX → I²C master firmware
├── lora_transmitter.ino     # LoRa TX → Pi serial interface firmware
...
- **LoRa Transmitter**: Upload `lora_transmitter.ino` to the Arduino connected to your Raspberry Pi.
- **LoRa Receiver**: Upload `lora_reciver.ino` to the Arduino connected to the dimmer node.
...
⚠️ **This project interfaces with high-voltage AC mains (100–230V).**
...
| Component | Status |
|-----------|--------|
| Prototype | ✔️ Complete |
| AC Dimming | ✔️ Fully tested on AC lighting |
| Scalability | ✔️ Scalable to multiple street lights |
| Backend/Hardware | ✔️ Stable |
